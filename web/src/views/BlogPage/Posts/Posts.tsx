...
const Post: React.FC<{ item: PostEdges; transition: any }> = ({ item, transition }) => {
  ...
  return (
    <S.Post>
      <Link to={`/news/${post.slug.current}`}>
        <div>
          <Heading as="h5" mb={4}>
            <Box as="mark" bg={post.categories[0].title !== 'News' ? 'quinary' : 'primary'}>
...
const Posts: React.FC<{ items: PostEdges[] }> = ({ items }) => {
  ...
  const itemTransitions = useTransition(items ? items : [], item => item.node.title, {
    from: {
      opacity: 0,
    },
    enter: {
